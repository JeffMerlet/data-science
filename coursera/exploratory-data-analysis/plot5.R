...
pm25SCCMotor <- subset(pm25SCC, grepl("*Onroad*", pm25SCC$Data.Category))

# Gets the data for Baltimore only
pm25SCCMotorBaltimore <- subset(pm25SCCMotor, fips == "24510")

# Summarizes the total of PM2.5 Emissions for each year
pm25SCCMotorBaltimoreSum <- ddply(pm25SCCMotorBaltimore,
                                  .(year),
                                  summarize,
                                  sum = sum(Emissions))
...
# Base plot
g <- ggplot(pm25SCCMotorBaltimoreSum, aes(x = year, y = sum)) + 
    geom_line()
# Theme
g <- g + theme_solarized() 
# Title
g <- g + ggtitle("Total PM2.5 Emissions from Motor Vehicule Sources For Baltimore")
